    ...
    <>
      <header className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto w-full max-w-6xl px-5 py-12 md:px-8 md:py-16">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-balance md:text-4xl">Framework Mapping</h1>
          <p className="mt-4 max-w-3xl text-sm leading-relaxed text-muted-foreground md:text-base">
            Mapping findings to shared vocabularies makes them comparable across teams and easier to prioritize
            against existing control programs.
          </p>
      ...
      <Section title="Full cross-reference" description="Every validated synthetic finding with its mappings.">
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full min-w-[980px] text-left text-sm">
            <caption className="sr-only">Findings mapped to OWASP, CWE, MITRE ATT&amp;CK, and NIST CSF 2.0</caption>
            <thead className="bg-surface-raised">
              <tr>
                {["ID", "Severity", "Finding", "OWASP Top 10", "CWE", "MITRE ATT&CK", "NIST CSF 2.0"].map((h) => (
      ...
      <Section title="Grouped views" description="Where the synthetic findings concentrate by vocabulary.">
        <div className="grid gap-4 lg:grid-cols-3">
          <Panel>
            <h3 className="text-sm font-semibold">OWASP Top 10 (2021)</h3>
            {owasp.length === 0 ? (
              <div className="mt-3"><EmptyState message="No OWASP mappings recorded." /></div>
            ) : (
              <ul className="mt-3 space-y-2">
                {owasp.map(([key, list]) => (
                  <li key={key} className="rounded-md border border-border bg-surface-raised px-3 py-2">
          ...
          <Panel>
            ...
            {cwe.length === 0 ? (
              <div className="mt-3"><EmptyState message="No CWE mappings recorded." /></div>
            ) : (
              <ul className="mt-3 space-y-2">
                {cwe.map(([key, list]) => (
                  <li key={key} className="rounded-md border border-border bg-surface-raised px-3 py-2">
          ...
          <Panel>
            ...
            {attack.length === 0 ? (
              <div className="mt-3"><EmptyState message="No ATT&CK techniques were conceptually relevant." /></div>
            ) : (
              <ul className="mt-3 space-y-2">
                {attack.map(([key, list]) => (
                  <li key={key} className="rounded-md border border-border bg-surface-raised px-3 py-2">
            ...
            <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
              Techniques are cited only where conceptually relevant to the synthetic finding. They describe adversary
              behaviour categories, not steps taken in this lab.
            </p>
          </Panel>
        </div>
      </Section>

      <Section title="NIST CSF 2.0 functions" description="High-level alignment of engagement activity to CSF 2.0 functions.">
